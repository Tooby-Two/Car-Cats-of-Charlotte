$(document).ready(function () {
    ...
    function loadGallery() {
        $.getJSON('data/tagToCharacterMapping.json', function (mapping) {
            const characterFiles = Object.entries(mapping)
                .filter(([character, info]) => info.folder === "car_cats" && info.subfolder === "daytona")
    ...
    function loadTimeline() {
        $.getJSON('data/worlds.json', function (worlds) {
            ...
            const timelineData = currentWorld.timeline.filter(event => event.region === "Daytona");
    ...
    function populateWorldCharacterLinks(worldName) {
        $.getJSON('data/characterLinks.json', async function (data) {
            ...
            const relationships = data.relationships.filter(rel => rel.world === worldName && rel.region === "Daytona");
    ...
    function loadLocations() {
        $.getJSON('data/worlds.json', function (worlds) {
            ...
            const locationsData = currentWorld.locations.filter(location => location.region === "Daytona");
